...
func _ready() -> void:
	...
	print(dialogue_data)
	dialogue_text($Control/container/day1/text, "Day 1.0")
...
func _on_next_pressed() -> void:
	if dialogue_data[current_dialogue].has("go to") && dialogue_data[current_dialogue]["go to"].size() > 1:
		dialogue_text($Control/container/day1/text, dialogue_data[current_dialogue]["go to"][1])
	else:
		emit_signal(dialogue_data[current_dialogue]["function"])

func _on_prev_pressed() -> void:
	if dialogue_data[current_dialogue].has("go to"):
		dialogue_text($Control/container/day1/text, dialogue_data[current_dialogue]["go to"][0])
	else:
		emit_signal(dialogue_data[current_dialogue]["function"])
