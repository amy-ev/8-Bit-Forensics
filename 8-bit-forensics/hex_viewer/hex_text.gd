...
func _draw() -> void:
	if get_parent().get_parent().name == "original_file":
		...
		var carved_buffer = carved_block
		var begin_offset:int = row_index * _row_width
		if begin_offset > len(carved_buffer):
			return
		...
		for visual_row in displayed_row_count:
			#the offsets for the rest of the visible rows
			var row_begin_offset: int = begin_offset + visual_row * _row_width
			if row_begin_offset >= len(carved_buffer):
				break
				
			# end of bytes_row
			var row_end_offset = row_begin_offset + _row_width
			if row_end_offset > len(carved_buffer):
				row_end_offset = len(carved_buffer)
			...
			var bytes_row = carved_buffer.slice(row_begin_offset, row_end_offset - 1)
			for i in len(bytes_row):
				var x = bytes_row[i]
				# if byte would be a space or del then change to a '.' 
				if x < 32 or x >= 127:
